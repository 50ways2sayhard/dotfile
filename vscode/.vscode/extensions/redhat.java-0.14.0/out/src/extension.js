        ...
        return vscode_1.window.withProgress({ location: vscode_1.ProgressLocation.Window }, p => {
            return new Promise((resolve, reject) => {
                ...
                languageClient.onReady().then(() => {
                    ...
                    vscode_1.commands.registerCommand(commands_1.Commands.COMPILE_WORKSPACE, () => {
                        return vscode_1.window.withProgress({ location: vscode_1.ProgressLocation.Window }, p => {
                            return new Promise((resolve, reject) => {
                                p.report({ message: 'Compiling workspace...' });
                                const start = new Date().getTime();
                                languageClient.sendRequest(protocol_1.CompileWorkspaceRequest.type, true).then((s) => {
                                    const elapsed = new Date().getTime() - start;
                                    const humanVisibleDelay = elapsed < 1000 ? 1000 : 0;
                                    setTimeout(function () {
                                        switch (s) {
                                            case protocol_1.CompileWorkspaceStatus.CANCELLED:
                                                reject(s);
                                                break;
                                            case protocol_1.CompileWorkspaceStatus.FAILED:
                                                reject(s);
                                                break;
                                            case protocol_1.CompileWorkspaceStatus.WITHERROR:
                                                reject(s);
                                                break;
                                            case protocol_1.CompileWorkspaceStatus.SUCCEED:
                                                resolve(s);
                                                break;
                                        }
                                    }, humanVisibleDelay);
