...
class Linter extends baseLinter.BaseLinter {
    constructor(outputChannel) {
        super('prospector', installer_1.Product.prospector, outputChannel);
    }
    runLinter(document, cancellation) {
        ...
        return new Promise((resolve, reject) => {
            utils_1.execPythonFile(document.uri, prospectorPath, prospectorArgs.concat(['--absolute-paths', '--output-format=json', document.uri.fsPath]), this.getWorkspaceRootPath(document), false, null, cancellation).then(data => {
                let parsedData;
                try {
