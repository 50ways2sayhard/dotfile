...
function promptToEnableAndConfigureTestFramework(wkspace, outputChannel, messageToDisplay = 'Select a test framework/tool to enable', enableOnly = false) {
    return __awaiter(this, void 0, void 0, function* () {
        const selectedTestRunner = yield selectTestRunner(messageToDisplay);
        if (typeof selectedTestRunner !== 'number') {
            return Promise.reject(null);
        }
        const configMgr = createTestConfigurationManager(wkspace, selectedTestRunner, outputChannel);
        if (enableOnly) {
            // Ensure others are disabled
            if (selectedTestRunner !== installer_1.Product.unittest) {
                createTestConfigurationManager(wkspace, installer_1.Product.unittest, outputChannel).disable();
            }
            if (selectedTestRunner !== installer_1.Product.pytest) {
                createTestConfigurationManager(wkspace, installer_1.Product.pytest, outputChannel).disable();
            }
            if (selectedTestRunner !== installer_1.Product.nosetest) {
                createTestConfigurationManager(wkspace, installer_1.Product.nosetest, outputChannel).disable();
            }
...
function displayTestFrameworkError(wkspace, outputChannel) {
...
function displayPromptToEnableTests(rootDir, outputChannel) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (item === yes) {
            yield promptToEnableAndConfigureTestFramework(vscode.workspace.getWorkspaceFolder(vscode.Uri.file(rootDir)).uri, outputChannel);
        }
        else {
...
function createTestConfigurationManager(wkspace, product, outputChannel) {
    const installer = new installer_1.Installer(outputChannel);
