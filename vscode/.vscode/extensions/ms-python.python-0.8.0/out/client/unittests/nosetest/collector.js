...
function discoverTests(rootDirectory, args, token, ignoreCache, outChannel, testsHelper) {
    ...
    return utils_1.execPythonFile(rootDirectory, configSettings_1.PythonSettings.getInstance(vscode_1.Uri.file(rootDirectory)).unitTest.nosetestPath, args.concat(['--collect-only', '-vvv']), rootDirectory, true)
        .then(data => {
